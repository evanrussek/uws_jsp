...
cdf %>% select(choice,correct_me,correct_response, CN,o1_val, o2_val)


# compare different correctness and what's chosen
cdf_m <- cdf %>%
  select(choice, correct_me, correct_response,trial) %>%
  melt(id = c("trial"))

ggplot(cdf_m, aes(x = trial, y = value)) + geom_point() + geom_line() + facet_wrap(~variable)
